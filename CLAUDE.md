...
**Play effects**: Note FX → SEQ ARP → Harmonize → MIDI Delay. Per-clip: each clip carries its own NOTE FX/HARMZ/SEQ ARP/MIDI DLY params (`clip_pfx_params_t`); switching clips loads that clip's params via `pfx_sync_from_clip`. `tN_pfx_reset` resets active clip's params atomically (including SEQ ARP). Scale-aware: noteFX_offset/harm_intervals/delay_pitch via `scale_transpose()` at render time; drum tracks bypass.

**SEQ ARP** (per-clip, melodic only): when `on=1`, `pfx_note_on/off` capture orig pitch+vel into the engine's held buffer (`arp_engine_t` in `play_fx_t`) instead of firing immediately; `arp_tick` runs once per master 96-PPQN tick from `render_block`, fires steps at `ARP_RATE_TICKS[rate_idx]` boundaries, and emits each note through the rest of the chain (NOTE FX → HARMZ → DELAY) via re-entrant `pfx_note_on/off` guarded by `fx->arp_emitting`. **Styles** (0–8): Up · Down · Up/Down · Down/Up · Converge (high/low/2nd-high/2nd-low/…) · Diverge · Play Order (insertion order) · Random · Random Other (no repeats until exhausted). **Octave expansion** (1–4) repeats the style-ordered list at +12 semitones per octave. **First-note clock sync**: empty→non-empty buffer sets `pending_first_note=1`; first emission waits until `(global_tick*24 + master_tick_in_step) % rate == 0`. **Steps** (0=Off/1=Mute/2=Skip): per-step velocity array (8 entries; UI deferred to phase 2 — DSP defaults to 100). Mute = rest at muted step but advance cycle; Skip = no fire and no cycle advance. **Vel decay** (-100..+100): `vel *= (1 + decay/100)^cycle_step_count`, resets at cycle wrap. **Gate** = `rate * gate_pct / 100`, clamped to `[1, rate-1]` so note-off fires before next note-on. `arp_silence(inst, tr)` drops held + silences sounding; called from `silence_track_notes_v2`, `seq_arp_on=0` toggle, `pfx_seq_arp_reset`, and `pfx_reset`.
...
| 4 SEQ ARP | 96 | On | Styl | Rate | Oct | Gate | Stps | Decy | — |
