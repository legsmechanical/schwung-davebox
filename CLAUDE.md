...
- **Cutting a release**: `./scripts/cut_release.sh <version>` (e.g. `0.2.0`). Requires clean tree, non-empty `[Unreleased]`, no existing `v<version>` tag. The script: finalizes CHANGELOG, bumps `release.json`, runs `build.sh` for a fresh tarball, commits, tags, pushes main + tag. You then upload `dist/davebox-module.tar.gz` to the GitHub release manually.
...
DSP state v=26 (only v=26 accepted); v≠26 → deleted + clean start. Full key list in `dsp/CLAUDE.md`.
