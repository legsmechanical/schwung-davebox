...
**NOTE FX bank** (bank 1): Rnd(K3, 0=Off/1-24=±semitone range, scale-aware). **Pitch random algorithms** (NOTE FX K3 and MIDI DLY K8): Shift+turn Rnd=cycle algorithm (Uniform/Gaussian/Walk); dialog shows while Shift held, commits on release. Walk: `note_random_walk` accumulator steps ±1 per note, clamps at ±range. State key `t%dc%d_nfra` / `t%dc%d_nfrm` (range/mode). Quantize(K5, melodic): `effective_tick_offset = raw*(100-q)/100`. **Drum tracks (NOTE/NOTEFX overlay)**: K6=Quantize (maps to `drum_lanes_qnt`, same setting as ALL LANES K6); value mirrored in `S.drumLaneQnt[t]` to survive `readBankParams` null-return from MIDI handler context.
...
**TRACK ARP bank** (bank 5, "ARP IN"): Styl(K1, 0=Off/1-9=style, drives tarp_on — no separate On knob), Rate(K2), Oct(K3), Gate(K4), Stps(K5), _(K6 empty), Sync(K7), Ltch(K8). tarp_on derived from tarp_style on load: `style!=0 → tarp_on=1`. State saves tarp_style only when `!=0`. **Latch shortcut** (Track View): Loop tap while holding ≥1 pad with TARP active toggles `tarp_latch` on/off without entering the bank; mirrors to `bankParams[t][5][7]`.
