...
**Views**: Track View (16 steps, isomorphic pads). Session View (CC 50, 4×8 grid) — **any clip pad press sets `activeTrack`** (carries to Track View). Loop held→pages view (`loopJogActive=false`); Loop+jog=±1 clip length + switches to step view (`loopJogActive=true`, showing OOB steps at 50% white via `OOB_SCRATCH_PALETTE` idx 50); `loopJogActive` cleared on every loop release. **NoteSession (CC 50) view switch**: tap=permanent toggle; hold=momentary (switches immediately on press, returns on release). Shift+Step15=double-and-fill (undoable on drum tracks; shows "LOOP DOUBLED" popup on all paths). **Step button LED palette**: beat-marker steps (i%4==0) use `BEAT_MARKER_PALETTE` (idx 49, RGB 26,26,26 = 10% white); OOB steps in step view use `OOB_SCRATCH_PALETTE` (idx 50, RGB 128,128,128 = 50% white); both initialized once in `drainLedInit` via `setPaletteEntryRGB`+`reapplyPalette`. **setPaletteEntryRGB limitation**: custom palette writes only reliably affect step button note LEDs at indices already defined in the Move's built-in palette (confirmed: 49, 50). Indices outside the ~49-58 range appear to use hardware defaults for note LEDs regardless of `setPaletteEntryRGB` writes — do not rely on palette customization for step LEDs outside this range.
...
**Suspend/Resume**: Back=suspend (sequencer keeps playing in background); Shift+Step13=resume (double-tap/long-press=direct, single-tap=Tools menu); Shift+Back=full exit. State auto-saved on suspend. `saveState()` helper used by suspend, Quit, and Shift+Back paths. **Suspend save coalescing fix**: on the suspend edge, UI sidecar is written immediately via `host_write_file`; `set_param('save')` is deferred to end of `tick()` via `S.pendingSuspendSave` flag so no subsequent set_param can overwrite it. **Set change while suspended**: on the resume edge (`!isSuspended && S._wasSuspended`), `active_set.txt` UUID is compared against DSP `state_uuid`; mismatch triggers `S.pendingSetLoad`. After `pendingDspSync` resync completes, `restoreUiSidecar(true)` re-applies the new set's sidecar (active track/clip/view/drums/perf) — necessary because `pollDSP()` overwrites `trackActiveClip` from DSP before the sidecar is applied. **SESSION LOADING...** OLED overlay shown while `S.stateLoading` is true (set when `pendingSetLoad` fires, cleared when `pendingDspSync` reaches 0).
