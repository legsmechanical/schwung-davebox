...
- **ROUTE_MOVE + external MIDI remap**: requires Schwung v0.9.8+. Silently bypassed if any chain slot has `forward_channel=THRU(-2)` — default for new slots. Set slots to AUTO(-1) or specific channel.
