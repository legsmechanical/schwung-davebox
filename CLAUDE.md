...
2. **Per-note tick offsets + 8-note polyphony** — replace per-step `int8_t step_tick_offset[256]` with per-note `int16_t note_tick_offset[256][8]`; bump `step_notes[256][4]` → `[256][8]`; update `fire_note_on_step` to schedule each note independently (deferred/lookahead per note, not per step); beat stretch scales all note offsets; state v=9; Nudge (K5) moves whole step as unit. Offset range ±(TICKS_PER_STEP-1) per note, symbolic — not hardcoded — so clip resolution can parameterise it later.
3. **Clip resolution** (per-clip TICKS_PER_STEP) — per-clip step size: 1/32·1/16·1/8·1/4·1/2·1-bar. Stored in `clip_t`. Changes render step-advance timing, beat stretch, recording offset math, Nudge range. Move Len+Res out of TRACK bank into a dedicated CLIP bank. Wire global menu recording offset clamp to clip's TICKS_PER_STEP.
4. **Step copy** (Track View, CC 60) — hold Copy, press source step (blinks white), press dest step → copy all step data (notes, gate, vel, all note offsets). Same clip only.
5. **Off-grid step LED** — dim track colour for steps with any non-zero note_tick_offset (deferred until per-note offsets land).
6. **Real-time scale quantize** — snap step notes to nearest scale degree at render time in `fire_note_on_step`.

### After unquantized-recording branch merges
7. **Undo/Redo** — step-level history.
8. **Drum mode** — pad_mode=drum, per-lane MIDI effects. DSP + JS build.
9. **Bank param snapshots** — 16-slot play effect bank snapshots (Session View, after drum mode).
10. **Arpeggiator** — Seq ARP + Live ARP, port from NoteTwist, one DSP phase.
11. **Swing** — wire global menu stub to DSP.
12. **MIDI clock sync** — external clock follow.

### V2 architecture (future)
**Absolute position + zoom (Deluge-style)**: replace step-anchor + offset model with notes stored as absolute tick positions within the clip. The 16 step buttons become a zoom window over the timeline — zoom out = each button covers more time, zoom in = finer resolution down to 1/64. Navigation via jog/scroll + zoom level. This is mathematically equivalent to the current `step_index × TICKS_PER_STEP + note_offset` model (just merged into one field), so migration is non-destructive. Informed by Synthstrom Deluge and OP-XY. Planned as a ground-up engine rewrite after current branch ships.
