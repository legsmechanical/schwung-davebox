...
**Transport**: Play/Stop. Shift+Play = restart (playing: `stop` then `play` next tick due to set_param coalescing; stopped: just `play`). Delete+Play: playing → `deactivate_all`; stopped → `panic` (clears all will_relaunch + queued). **Do not use per-track `tN_deactivate` for bulk clearing** — DSP processes one per audio callback; pollDSP restores stale state between calls. BPM owned by SEQ8 after init; `set_param("bpm")` updates `tick_delta` + `cached_bpm`.
...
**SEQ ARP** (per-clip, last stage of pfx chain): captures every note-on/off emission from upstream stages (immediate output of NOTE FX → HARMZ + queued delay echoes) into the engine's held buffer (`arp_engine_t` in `play_fx_t`) and re-emits picks raw at its own rate. The intercept lives in `pfx_send`: when `arp.on && !arp_emitting`, status `0x90`/`0x80` route to `arp_add_note`/`arp_remove_note` instead of MIDI out (CC and other messages pass through). All input sources (sequencer, live pad, external MIDI) flow into the same chain — there is no source-aware bypass. The arp emits its picked pitch via `pfx_send` with `arp_emitting=1` so the pick goes raw to MIDI out, no further processing. `arp_tick` runs once per master 96-PPQN tick from `render_block`. **Styles** (0–8): Up · Down · Up/Down · Down/Up · Converge (high/low/2nd-high/2nd-low/…) · Diverge · Play Order (insertion order) · Random · Random Other (no repeats until exhausted). **Octave expansion** (1–4) repeats the style-ordered list at +12 semitones per octave. **First-note clock sync**: empty→non-empty buffer sets `pending_first_note=1`; first emission waits until `(global_tick*24 + master_tick_in_step) % rate == 0`. **Steps** (0=Off/1=Mute/2=Step): per-step level array (8 entries, levels 0..4; 0=step off, 1=row 0/vel 10, 4=row 3/incoming vel; intermediate levels lerp). Mute = rest at level-0 step but advance cycle; Step = skip (no fire, no cycle advance). Step pattern is musically anchored: editor column = `(global_master_pos / rate) % 8`, so column 0 always fires on the first rate division of the bar. **Vel decay** (-100..+100): `vel *= (1 + decay/100)^cycle_step_count`, resets at cycle wrap. **Gate** = `rate * gate_pct / 100`, clamped to `[1, rate-1]` so note-off fires before next note-on. `arp_silence(inst, tr)` drops held + silences sounding via raw `pfx_send`; called from `silence_track_notes_v2`, `seq_arp_on=0` toggle, `pfx_seq_arp_reset`, and `pfx_reset`.
...
| 5 SEQ ARP | 97 | On | Styl | Rate | Oct | Gate | Stps | Decy | — |
...
| `tN_pfx_snapshot` | get | 24 space-sep integers | Batch read of active clip's pfx params: fields 0-16 are NOTE FX K0-K4, HARMZ K0-K3, MIDI DLY K0-K7 (legacy 17); fields 17-23 are SEQ ARP K0-K6 (on/style/rate/oct/gate/steps/vel_decay). JS parser is length-aware. |
| `tN_seq_arp_*` | set | varies | Per-clip SEQ ARP params: `seq_arp_on` (0/1), `seq_arp_style` (0..8), `seq_arp_rate` (0..9 → 1/32..1-bar), `seq_arp_octaves` (1..4), `seq_arp_gate` (1..200%), `seq_arp_steps_mode` (0=Off/1=Mute/2=Skip), `seq_arp_vel_decay` (-100..100). Toggling on→off silences sounding output and clears held buffer. |
...
typedef struct {
    ...
    clip_pfx_params_t pfx_params; /* per-clip NOTE FX/HARMZ/SEQ ARP/MIDI DLY params (24 ints + 8B step_vel). */
