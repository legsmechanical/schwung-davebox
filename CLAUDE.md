...
**Global menu** (Shift + CC 50): `drawHierarchicalMenu`; jog rotate = navigate; jog click = CC 3 (0xB0 d1=3) = edit via `handleMenuInput`; Back = exit. Items: **BPM** (40–250, real-time jog, linear ±1/detent, CC 14 intercepted in `onMidiMessageInternal`), **Key** (wired), **Scale** (wired), **Scale Aware** (on/off, default on), **Launch** (`set_param("launch_quant", "0"–"5")` = Now/1/16/1/8/1/4/1/2/1-bar), **Input Vel** (0=Live pass-through, 1–127=fixed; persisted via DSP `input_vel`; `effectiveVelocity()` applied to all pad note-ons), **Inp Quant** (stub, session-local — ON snaps live recording timing to grid; OFF captures raw tick_offset), **Save+Unload** (stub), Swing Amt/Res (stub).
...
4. **Off-grid step LED** — dim track colour for steps with any non-zero note_tick_offset (deferred until per-note offsets land).
