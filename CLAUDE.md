...
1. **Drum mode (in progress)** — step-edit overlay (Vel+Dur in drum mode, K3 touch gate LED overlay), per-lane mute/solo, DRUM SEQ K2/K3 per-lane (clock shift + nudge), DRUM SEQ Shift+K4 resolution zoom per-lane, pfx banks (NOTE FX/HARMZ/MIDI DLY) wired to selected lane's `pfx_params` + `refreshDrumLaneBankParams`, Copy+step drum lane branch (`tN_lL_step_S_copy_to`), undo/redo drum step resync (`pendingDrumResync` after `undo_restore`), velocity pad same-level toggle to live-velocity, live recording (`onMidiMessageExternal` drum routing + pad recording + echo guard), mode-switch confirmation dialog + `tN_convert_to_drum` / `tN_convert_to_melodic`, UI sidecar `activeDrumLane[]`.
