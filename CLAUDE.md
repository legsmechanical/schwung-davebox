...
**Views**: Track View (16 steps, isomorphic pads). Session View (CC 50, 4×8 grid) — **any clip pad press sets `activeTrack`** (carries to Track View). Loop held→pages view; Loop+jog=±1 clip length. **NoteSession (CC 50) view switch**: tap=permanent toggle; hold=momentary (switches immediately on press, returns on release). Shift+Step15=double-and-fill (undoable on drum tracks; shows "LOOP DOUBLED" popup on all paths).
