...
**Looper — mid-LOOPING rate change queue** (commit `8cd5b28`, gap fix `9800a19`): get a loop running at 1/4 bar (step 4). While it's looping, press step 5 (1/2 bar) without releasing 4. The new rate should be **queued** — the current 1/4 cycle finishes, then goes directly to CAPTURING at the new rate (no ARMED sync gap). Release step 5 while still holding 4 → falls back to 1/4 rate at the next boundary. Try a quick stab of step 5 → loop keeps going at 1/4 with no glitch.
