...
**Views**: Track View — 16 step buttons = active clip page; pads = isomorphic 4ths diatonic; Shift+bottom-row = track select; OOB steps = White; playback head = White. Loop (CC 58) held → pages view: pages with notes pulse, empty pages = solid track color, out-of-clip = DarkGrey. Session View (CC 50) — 4×8 pad grid; jog/Up(54)/Down(55) scroll; Shift+step or side buttons (CC 40–43) = launch scene. **Knob LEDs (CC 71-78)**: Session View = active-track indicator (White); Track View + bank 2/3/4/5 = dirty-param indicators (White when param ≠ default). **Clip buttons (CC 40–43, Track View)**: focused = bright track color; non-focused inactive-only = DarkGrey; with active notes = dim track color; empty = off.
...
| `tN_cC_hard_reset` | set | any | Full factory reset via `clip_init` (length=128, tps=24, all steps/notes/pfx cleared). Undo snapshot, silence, pfx_sync if active clip. Clip stays in transport. Saves state. |
