...
**Performance Effect Mode** (Session View only, requires loop running): double-tap Loop to lock Perf Mode, or hold Loop + press a length pad to start. Pad grid switches to mod layout — R0 (68–75): length pads 1–6 (1/32..1bar) + Latch (75); R1 (76–83): pitch mods; R2 (84–91): vel/gate mods; R3 (92–99): wild mods. Eight modifiers: **R1** Oct↑ (bit0, +12st), Scale↓ (bit1, −1 scale degree, scale-aware) · **R2** Cresc (bit2, +13vel/cycle), Soft Decay (bit3, vel×0.5), Staccato (bit4, gate→cap/8) · **R3** Halftime (bit5, suppress every odd cycle), Glitch (bit6, random ±5st), Sparse (bit7, ~50% suppress). Pads momentary by default; hold = active (magenta/yellow/cyan per row). **Latch** (R0 pad 8, note 75): tap → enter Latch mode (pad yellow, mods toggle-on on press); tap again → exit Latch, clear all toggled mods. Long-press Latch clears all toggled mods without exiting. **Step buttons = 16 preset snapshot slots**: slots 1–8 pre-loaded with factory presets (Float·Sink·Heartbt·F.Dust·Robot·Dissolve·Chaos·Lift); tap → recall (step goes White, mod LEDs update), tap same → clear; Shift+tap → save current mod state to slot; existing preset slots show PurpleBlue. Recalled + held + toggled mods compose by OR. **Lock**: double-tap Loop → `perfViewLocked=true`; Loop LED blinks White at 1/8 rate; all loops + mods persist after Loop release; single-tap Loop while locked → unlock + stop. Track View switch always unlocks. All exit paths clear `perfRecalledMods/Slot`. **DSP**: `perf_apply()` runs in `pfx_send` for every looper-emitted event; note-offs always look up `perf_emitted_pitch[tr][raw]` for correct pitch matching; `looper_pending_silence` deferred flag drains at `looper_tick` for ROUTE_MOVE safety; `looper_cycle` increments each loop wrap. Drum-track events bypass pitch mods (Oct↑, Scale↓, Glitch). Staccato uses `perf_staccato_notes[16]` pending queue, fires gate-end at `looper_pos == fire_at`. OLED takeover in Perf Mode: shows "PERF MODE" header, active rate + mod names, preset name/slot when recalled. JS state: `perfModsToggled | perfModsHeld | perfRecalledMods` sent as `perf_mods` each change.
...
## Pending tests (deployed but not yet verified on device)

The following changes were built, deployed, and committed but have not been
verified on hardware. Run through the checklist below before relying on them.

**SEQ ARP overhaul** (commit `28086ef`):
1. **Style=Off bypass**: select SEQ ARP bank, set K1 to Off — confirm sequencer + live pads pass straight through (no arp). Increment K1 once → "Up" — confirm arp engages.
2. **Style→Off silence**: while arp is sounding, set K1 back to Off — confirm any sounding note silences immediately, no stuck note.
3. **Octaves bipolar / skip-zero**: rotate K3 CCW from `+1` — confirm value sequence is `+1, -1, -2, -3, -4` (no `0`). Rotate CW from `-1` → confirm `-1, +1, +2, +3, +4`. Hold a chord and confirm `+2` ascends two octaves, `-2` descends two octaves.
4. **Retrigger=On**: hold a chord; while chord is held, add a new note — pattern column should jump back to step 1. Let the active clip wrap (run a 16-step clip) — pattern should restart at column 1 each loop.
5. **Retrigger=Off**: same scenarios as above — pattern should *not* restart; column position should stay clock-anchored.
6. **Steps Mode editor moved to K5**: enter SEQ ARP bank, set K5 to `Mut` or `Stp`, hold K5 — pad grid should become the 8×4 vel-level editor (was K6).
7. **Dirty LED indicators**: confirm K1/K3/K6 LEDs are unlit at default (Off / +1 / On) and light White when changed.
8. **Per-clip persistence + load**: set non-default values, switch clips, switch back — values should restore. Save (Shift+Back), reboot Move — values should reload.
9. **Vel Decay gone**: confirm there is no Decy knob and turning K7 does nothing in SEQ ARP bank.
10. **Free-running stopped clock**: stop transport, hold a chord — confirm arp still runs (covered by earlier `4ad22c3`, but worth re-verifying after the overhaul).

**Shift+Play restart** (commit `6ecbf2b`): repeatedly press Shift+Play during playback — should reliably reset to step 0 every press (the previous cross-tick stop+play approach was unreliable).

**Delete+Play swap** (commit `a3be1bd`): playing → all clips deactivate; stopped → panic + clears will_relaunch.

**Free-running SEQ ARP clock** (commit `4ad22c3`): with transport off and SEQ ARP on, holding a chord on the pads should produce the arpeggiated output continuously.

**Global MIDI Looper — basic capture/loop** (commit `93e50ab`): in Session View, hold Loop — confirm steps 1–6 pulse white↔purple at their respective rates (1/32 fastest, 1bar slowest); step 16 stays solid purple, toggling to a 1/8t blink when tapped (triplet mode). With transport playing (any clip with notes), then hold Loop+step 4 (1/4 bar). After the next quarter-note boundary, the looper should capture 1/4 bar; once capture completes, the captured 1/4 bar should loop while the underlying tracks (any with TRACK K8 Lpr=On) are silenced. Release either Loop or step 4 → looper stops cleanly, silenced tracks resume, no stuck notes. Toggle TRACK K8 Lpr=Off on a track and try again — that track should pass through unchanged during loop, never captured nor silenced. Try mid-capture release: hold less than the full capture length → nothing should loop (silent discard). Test with transport stopped: same gesture should still arm/capture/loop using live pad input only (the looper clock is the free-running `arp_master_tick`, independent of transport).

**Looper — mid-LOOPING rate change queue** (commit `8cd5b28`): get a loop running at 1/4 bar (step 4). While it's looping, press step 5 (1/2 bar) without releasing 4. The new rate should be **queued**, not snap immediately — the current 1/4 cycle finishes, then transitions to ARMED at 1/2 bar (waiting for next master-clock 1/2-bar alignment), captures fresh, and loops at 1/2. Release step 5 while still holding 4 → falls back to 1/4 bar at the next loop boundary. Try a quick stab of step 5 (press+release before the loop boundary fires) → loop should keep going at 1/4 with no audible glitch (the queue gets cancelled by the release re-arming with rate=current). Try Loop+step 4 → loop → Loop+step 6 (1bar): should switch to 1bar rate at the 1/4 boundary, then ARMED waits up to 3/4 bar for absolute 1bar alignment, then captures + loops 1bar. The gap is expected.

**Looper — double-tap Loop to lock view** (commits `527b98b` / `46bb2d2`): in Session View, double-tap Loop quickly. The looper view should "lock" — Loop button blinks White at 1/8 rate, length-button pulses keep showing, and any active loop keeps playing. Release Loop and the view stays alive. Test: tap a length pad while locked → loop arms+captures+plays normally. Single-tap Loop → unlock, loop stops, view returns to normal Session View. Switch to Track View while locked → also unlocks + stops cleanly. Long-press Loop while locked → no-op (lock persists).
...
11. **Perf Mode Phase 3** — full 24-modifier set (8 pitch, 8 vel/gate, 8 wild) across R1/R2/R3. Phase 1+2 shipped (8 mods + preset snapshots, verified). See memory file for full mod list.
...
Perf Mode keys: `perf_mods` (set, uint32 bitmask — OR of bit0=Oct↑, bit1=Scale↓, bit2=Cresc, bit3=Soft Decay, bit4=Staccato, bit5=Halftime, bit6=Glitch, bit7=Sparse; JS ORs toggled+held+recalled and sends on every change).
