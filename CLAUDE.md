...
**loop-double-fill**: Shift+Loop (Track View) — doubles active clip length and copies all step data from the first half into the newly created second half (`tN_loop_double_fill` DSP set_param: `undo_begin_single` snapshot, memcpy all step arrays offset by `len`, update `cl->length`, `clip_migrate_to_notes`, `seq8_save_state`). JS: checks `clipLength * 2 > 256` first — if so shows "CLIP FULL" pop-up and aborts; otherwise updates `clipLength` mirror, sets `pendingStepsReread=2`, calls `refreshPerClipBankParams`. Undo restores original length and step data. Does not consume Loop-hold state (`loopHeld` remains false).

**resolution-zoom**: Shift+CLIP K4 — absolute mode: note absolute tick positions do not move; only `ticks_per_step` changes and clip length in steps adjusts to preserve total tick duration (`tN_clip_resolution_zoom` DSP set_param: guard recording, compute `new_len = ceil(length*old_tps / new_tps)`, block if `new_len > 256`, update `cl->ticks_per_step` + `cl->length`, remap `current_step`/`tick_in_step` via absolute clip tick, call `clip_build_steps_from_notes` + `seq8_save_state`). JS: intercepts `shiftHeld && pm.dspKey === 'clip_resolution'`; computes `_new_len = ceil(clipLength * old_tps / new_tps)` — shows "NOTES OUT / OF RANGE" pop-up if `> 256`, silently blocks if step held or recording armed; otherwise updates `bankParams`, `clipTPS`, `clipLength` mirrors, clamps `trackCurrentPage`, fires `tN_clip_resolution_zoom`, sets `pendingStepsReread=2`, calls `refreshPerClipBankParams`. No undo (consistent with normal clip_resolution).

**route-move-external-midi**: ROUTE_MOVE tracks: `onMidiMessageExternal` never calls `liveSendNote` (injecting causes echo cascade → crash; Move handles cable-2 natively). Pads still inject (cable-0). Recording from external MIDI works via `recordNoteOn` set_param path; `seqActiveNotes.has(d1)` guard prevents sequencer playback echoes from being re-recorded at loop end. Channel remapping not possible from JS; requires shim `/schwung-ext-midi-remap` feature (see SCHWUNG_SEQ8_LIMITATIONS.md §12–13).
...
**step-gate-span + recording-gate-fix + ui-state-sidecar**: Hold step (with notes) + tap another step → sets held step's gate to span the distance (right-tap: `dist=tapped-held`; left/wrap: `dist=len-held+tapped`; `gate=dist*tps`, clamped 1–65535). Suppresses first-step release toggle via `stepBtnPressedTick[heldStepBtn]=-1; stepWasHeld=true` inside the gate-set block. LED overlay in `updateStepLEDs()`: while step held with notes, buttons from start step through gate-end boundary lit `TRACK_DIM_COLORS[activeTrack]`; K3 (Dur) gate overlay overwrites when Dur touched. Recording gate fix: removed `suppress_until_wrap` from note scan in both `record_note_off` and `finalize_pending_notes` — `clip_migrate_to_notes` (called from 20+ set_param handlers, including `_gate`) hardcodes `suppress_until_wrap=0` on rebuild, so the old check caused gate to stay at GATE_TICKS=12 placeholder whenever any step operation ran during recording; pitch+tick+active is specific enough without the flag. UI sidecar: Shift+Back saves `{v,at,ac,sv}` to `seq8-ui-state.json` alongside the DSP state file (`uuidToUiStatePath`); `init()` restores `activeTrack`, `trackActiveClip[]`, `sessionView`; Clear Session also wipes sidecar + shows "SESSION CLEARED" pop-up. First-run defaults: when no sidecar exists, `pendingDefaultSetParams=[{scale_aware,1},{metro_vol,100},{input_vel,0}]` drains one per tick after state fully settled (`pendingDspSync==0`); `uiDefaultsApplyAfterSync` flag re-applies them after a `state_load` re-sync.
...
**transport-fixes + now-mode + session-focus + ui-polish**: `silence_track_notes_v2` called in stop/panic handlers (was missing `play_pending[]` flush — caused stuck notes with MIDI Delay/Harmonize) · "Now" launch preserves `tick_in_step` continuity across clip switch instead of resetting to 0 (was rewinding up to `tps-1` ticks, causing audible pause + double-note retrigger) · Session View clip pad press always sets `activeTrack` (previously only the new-launch branch did) · `clipTPS[dst]` mirror updated in `copyClip`/`copyRow` before `refreshPerClipBankParams` (was showing stale 1/16 resolution after paste) · `drawTrackRow(y)`: active track gets outline box (10×12px, 1px border + 1px pad), muted tracks get filled box (all tracks, inverted number), soloed tracks blink number only; no underline · Global Menu: "Clear Sess" action — confirmation dialog (jog toggles No/Yes, jog click confirms, Back cancels); on confirm writes `{"v":0}` to state file + triggers `state_load` for DSP fresh-init; resets JS-only state (SEQ ARP, clipSeqFollow, activeBank, undo snapshots).

**code-organization**: sens=16 baseline for knobs, fine-tuned per-knob · `dsp/seq8_set_param.c` via `#include` (single translation unit) · `ui_constants.mjs` (ES module) for constants/palette/fmt/MCUFONT/`SCALE_DISPLAY`.

**drum-display + oled-polish**: Bank headers use `[ LABEL ]` format · `Knob:` label at y=0 (top) on all track views · Melodic idle: second row shows `Oct: +X` left + right-aligned key+scale (full names via `SCALE_DISPLAY`; abbreviated only for H Minor/M Minor/Pent Major/Pent Minor) with 1px underline when `scaleAware` is on · Octave overlay removed (info lives on idle row) · Track numbers and clip letters evenly distributed across full 128px width (`t*16+5`) · `drawTrackRow(y)`: active track gets outline box (10×12px, 1px border + 1px pad around character), muted=filled box (all tracks), soloed=blink number only · Drum NOTE FX bank overview: K7 (Oct ±12 coarse) + K8 (Note ±1 fine) control lane MIDI note via a merged boxed block in the bottom-right corner — visually distinct from the real-time NOTE FX params above it; box outline at rest, inverted fill on touch; shows "Oct" / "Note" labels centered in each half and `midiNoteName(note) + ' ' + note` centered below · `_lane_note` DSP handler now calls `seq8_save_state` (was missing — lane notes were lost on reload) · `pendingDrumLaneResync` (lightweight resync: re-reads active lane steps only, 3 IPC calls) replaces `pendingDrumResync` for drum SEQ K2 (clock shift) and K3 (nudge) — was triggering `syncDrumLanesMeta` = 64 IPC calls, causing audible lag.

**drum-mode**: `pad_mode=1` per track. DSP: `drum_lane_t` = `clip_t clip + uint8_t midi_note`; `drum_clip_t` = `drum_lane_t lanes[32]`; `drum_clips[NUM_CLIPS]` + `drum_current_step[32]` + `drum_tick_in_step[32]` in `seq8_track_t`. `drum_track_init` initializes all 32 lanes per clip (`clip_init + midi_note = DRUM_BASE_NOTE + l`). Render path parallel to melodic: one note per lane per step, no chord. JS: left 4×4 pads = lanes 0–15 (or 16–31 on page 2); right 4×4 = velocity zones. `drumLaneSteps[t][l]` / `drumLaneHasNotes[t][l]` / `drumLaneNote[t][l]` / `drumLaneLength[t]` / `drumStepPage[t]` / `drumLanePage[t]` / `activeDrumLane[t]`. `drumClipNonEmpty[t][c]` from `tN_cC_drum_has_content`. `tN_drum_active_lanes` bitmask getter: uint32, one bit per lane, set when lane has a hit at its current step — used for pad flash during playback. `drumLaneFlashTick[t][l]` + `DRUM_FLASH_TICKS=8` for pad flash. Session/track view: drum clips with content show `TRACK_DIM` (same as parked melodic); playing/queued = bright/flash; empty = off. Loop-held in drum Track View shows page overview using `drumLaneSteps` data. `syncDrumLaneSteps(t,l)` reads `tN_lL_steps` (active-clip-implicit getter) — deferred 2 ticks after clip switch via `pendingDrumResync` because DSP must process `tN_launch_clip` first (contrast: melodic `tN_cC_steps` is clip-indexed and reads immediately). `pendingDrumLaneResync` / `pendingDrumLaneResyncTrack` / `pendingDrumLaneResyncLane`: lightweight variant — re-reads active lane steps only (3 IPC calls); used by drum SEQ K2/K3 after each detent to avoid 64-call `syncDrumLanesMeta`. `tN_lL_step_S_toggle` / `tN_lL_step_S_clear` / `tN_lL_step_S_vel` / `tN_lL_step_S_gate` — drum lane step setters, same note machinery as melodic. Lane MIDI note: `tN_lL_lane_note` get/set (0–127); drum NOTE FX K7 (Oct, ±12/detent) and K8 (Note, ±1/detent) adjust it, clamped 0–127, no cycling; persisted via `t{t}c{c}l{l}_mn` (sparse, omitted when = `DRUM_BASE_NOTE + lane`). `DRUM_BASE_NOTE=36`, `DRUM_LANES=32`.
...
**JS file structure**: `ui.js` (main, ~3597 lines) + `ui_constants.mjs` (hardware constants, palette, fmt functions, MCUFONT, pixelPrint — 206 lines). Both must be deployed together. `build.sh` copies both automatically. JS-only deploy requires copying both (see above).

**DSP file structure**: `dsp/seq8.c` (~2238 lines) includes `dsp/seq8_set_param.c` (~1748 lines) via `#include`. Single translation unit — no extern declarations, no build changes. `seq8_set_param.c` covers all `set_param` handlers.
