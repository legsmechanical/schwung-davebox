...
| [#72](https://github.com/charlesvestal/schwung/pull/72) | `f34726e5` | `src/host/shadow_midi.c` | Hold inject drain for 3 frames (~9ms) after overtake exit — prevents SIGABRT when suspending (Back) with a ROUTE_MOVE drum pattern playing |
...
- **ROUTE_MOVE suspend crash**: suspending (Back) while a ROUTE_MOVE drum pattern is playing caused SIGABRT in the shim — DSP note-offs raced Move firmware during overtake exit. Fixed in Schwung PR #72 (commit f34726e5, `shadow_drain_midi_inject` exit_hold).
