...
**NOTE FX bank** (bank 1): Rnd(K3, 0=Off/1-24=±semitone range, scale-aware). **Pitch random algorithms** (NOTE FX K3 and MIDI DLY K8): Shift+turn Rnd=cycle algorithm (Uniform/Gaussian/Walk); dialog shows while Shift held, commits on release. Walk: `note_random_walk` accumulator steps ±1 per note, clamps at ±range. State key `t%dc%d_nfra` / `t%dc%d_nfrm` (range/mode). Quantize(K5): `effective_tick_offset = raw*(100-q)/100`.
