...
- **Two changelogs, both `[Unreleased]`** — for every `feat:` or `fix:` commit, add entries under the appropriate subsection (`### Features` / `### Fixes` / `### Performance / UX` / `### Documentation`) in **both**:
  - `CHANGELOG.md` (tracked, **user-facing**) — a short, plain-language entry. This is what ships as the public release notes.
  - `notes/tech-changelog.md` (untracked, gitignored via `notes/`, **technical**) — the full technical detail: DSP keys, struct names, root-cause/fix mechanics, sidecar version bumps. The deep archive of why/how.
  `scripts/cut_release.sh` finalizes **both** `[Unreleased]` sections into versioned headings at release time. An empty `CHANGELOG.md [Unreleased]` aborts the cut; a missing/empty `notes/tech-changelog.md` only warns (it's local-only — never blocks a release).
- **Manual = edit the DRAFT, not the public file.** The user-facing manual ships pinned to releases. Edit **`MANUAL.draft.md`** (tracked working copy, repo root) — *not* `MANUAL.md` — whenever a `feat:`/`fix:` commit changes user-visible behavior (controls, displays, pad/button semantics, persistence, workflows), in the same commit. `MANUAL.md` is the **published** manual and is updated **only** by `scripts/cut_release.sh`, which promotes `MANUAL.draft.md` → `MANUAL.md` (stripping the draft banner) at release time, so the public manual never documents unreleased features. Never edit `MANUAL.md` by hand. Skip the draft for internal-only changes (refactors, DSP plumbing, build, debug logging). When ambiguous, ask the user. (Mirrors the CHANGELOG/tech-changelog split; the staging-manual workflow was set up 2026-06-19.)
- **Cutting a release**: `./scripts/cut_release.sh <version>` (e.g. `0.2.0`). Requires clean tree (including no untracked files — park untracked `notes/` files aside if needed, but NOT the tracked `CHANGELOG.md`, which the release reads and finalizes), non-empty `[Unreleased]`, no existing `v<version>` tag. The script: finalizes both `CHANGELOG.md` and (best-effort) `notes/tech-changelog.md`, bumps `release.json` *and* `module.json` (atomic — Module Store update detection compares installed `module.json` against repo `release.json`), runs `build.sh` for a fresh tarball, commits, tags, pushes main + tag. After it succeeds, publish the release with condensed user-facing notes: `python3 scripts/condense_changelog.py <ver> > dist/release-notes-v<ver>.md` → `gh release create v<ver> dist/davebox-module.tar.gz --title "v<ver>" --notes-file dist/release-notes-v<ver>.md`. Then `./scripts/draft_announcement.sh <ver>` copies a Discord-pasteable announcement to the macOS clipboard for manual paste into the Schwung Discord release channel.
