...
Current base: **`a73ebe89`** (upstream `charlesvestal/schwung` `main` HEAD; 2026-05-22), branch `main` on the `legsmechanical/schwung` fork. The fork was rebased onto upstream `main` on 2026-05-22, so the local patch (`patches/davebox-local.patch`, `git diff a73ebe89..main -- src/`) is now **co-run-only** — 801 lines: chain-edit + Move-native co-run, incl. the selective LED filter + track-button strip + co-run LED handoff + chain-edit knob exposure (knob turn+touch). As of 2026-05-22 the co-run input split is **generalized** — a tool-declared manifest (`corun_keep_mask` + the shared `corun_group_for_event` map + `shadow_corun_begin/end` API) replaces the old hard-coded nav-CC lists; dAVEBOx's split is the zero-config default (`keep_mask==0`). This form is the basis for an upstream PR (`notes/corun-upstream-pr-draft.md`). The May-22 LED snapshot/restore set + PR #93 `midi_send_external` are upstream as of `a73ebe89` and no longer carried locally (earlier inject-race / `EXT_MIDI_REMAP_BLOCK` / cable-2 routing were upstreamed back in v0.9.13). `shadow_control_t` reserved bytes carry `midi_indicator_enabled` (upstream) + `corun_chain_edit_slot` + `corun_move_native_track`, `reserved[3]`.
