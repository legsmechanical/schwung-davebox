...
**2026-06-10 — co-run view-addressing mechanism added to fork (`corun-view-addressing`).** **Co-run-specific, but view-agnostic + upstreamable.** Generalizes co-run beyond the two hardcoded targets (`CORUN_TARGET_CHAIN_EDIT` / `CORUN_TARGET_MOVE_NATIVE`) so a co-running tool can open any *registered* Schwung screen as a temporary **overlay** over its current co-run target and return — without changing `corun.target`, so the consumer tool never tears down. Three **addressing verbs** are defined on the shared QuickJS `globalThis` by `shadow_ui.js`: `shadow_corun_entries()` (discovery — returns the openable screen ids so a tool can capability-gate per-entry), `shadow_corun_open(id, keep_mask, args)`, and `shadow_corun_close()`. They are backed by a curated **`CORUN_ENTRIES` registry** (id → `{ enter, keepDefault, overlay }`) that reuses existing screen enter-functions and is **never** auto-derived from the `VIEWS` enum. The only C addition is one SHM helper, **`shadow_corun_overlay(active, keep_mask)`**, which flips `shadow_display_owner` (→ `SCHWUNG_UI`) and applies the keep_mask **without touching `corun.target`** (JS can't write those `shadow_control` fields); overlay nav is routed in shadow_ui's `onMidi` dispatcher with the `suspend_keeps_js` Back guard excluded while an overlay is open (Back navigates within / closes the overlay rather than suspending the tool). **Catalog split:** upstream registers `slots` / `chain_editor` / `master_fx` / `global_settings`; the fork adds **`fx_picker`** in a fork-guarded block. First consumer is dAVEBOx's **Note/Session button in Move co-run**, which opens `fx_picker` (Master FX · Send A · Send B · Move-track insert FX 1–4) over the Move synth, keeps the jog group (`DAVEBOX_PICKER_KEEP_MASK`) while open, and capability-gates on `shadow_corun_entries()` including `fx_picker`. Mechanism docs in `docs/CORUN.md` (fork). Code-complete; pending on-device verification.
