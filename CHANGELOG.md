...
### Performance / UX
- **Performance View knob LEDs show looper state.** In Performance View, each knob LED lights in its track color when that track's looper is on, and goes dark when off. **Shift + knob touch** toggles the looper for that track with a `LOOPER ON / TRACK N` popup. The track menu reflects the change.
