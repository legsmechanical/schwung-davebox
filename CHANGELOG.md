...
- **Conductor tracks — real-time, non-destructive transposition.** A new track type (Track Config → Type → **Cond**) that transposes all playing melodic clips up or down in real time, driven by the note the Conductor plays — sequence a progression and every responding track follows it in key. Zero shift is the root at octave 4; play higher/lower to shift up/down, scale-aware (by scale degree) or chromatic (by semitone) following the global Scale-Aware setting, with the Conductor's pad octave scaling the move. The clips' written notes are never changed — the shift is live and reversible. One Conductor per session; it emits no MIDI of its own, keeps its track color, and its Channel/Route are inert (Mute pauses conducting → responders snap to written pitch; Solo disabled). Converting to/from Conductor keeps your sequenced notes (note/duration/iteration/probability) and resets FX/ARP/automation. The jog cycles five banks: **Conduct** (adds **CdLk** — gate-hold vs sample-and-hold lock), **NoteFX** (Octave/Offset/Random shaping the Conductor's own note before the shift), **Responder** (per-track on/off, default on), **Octave** (per-track −4…+4 added on top), and **When** (per-track **Next** vs **Now** held-note transition). Empty Conductor step or Mute → responders play their written pitch; drum tracks never respond. Per-clip Responder/Octave/When/CdLk settings persist with your set.
- **Scene bake can apply the Conductor.** When you bake a scene (all tracks at one clip index) and a Conductor exists with responders on at that scene, the bake flow adds an **"Apply Conductor?" — YES / NO / CANCEL** step (after Wrap). YES folds the Conductor's transposition permanently into each responder clip — honoring gate-hold vs CdLk Lock, the Conductor clip's Iter/Prob trig conditions, NOTE FX, and per-track Octave — then auto-disables that Conductor clip's Responder flags for the baked tracks so live playback doesn't double-apply. NO bakes written pitches; CANCEL aborts. Clip bake (single track) has no Conductor option. Probability/NOTE-FX random re-roll per occurrence (freeze behavior).
- **Ableton export can apply the Conductor (non-destructive).** When a Conductor exists, the export confirm adds an **"Apply Conductor?" — YES / NO / CANCEL** step. YES folds the Conductor's transposition into each exported responder clip — only where that scene's Conductor clip has notes and the track responds — reusing the same chain as scene bake (gate-hold vs CdLk Lock, Iter/Prob, NOTE FX, per-track Octave) plus polymeter auto-extend so multi-page Conductors are fully captured. NO (default) exports written pitch; CANCEL aborts the export. The live session is never modified. The Conductor track itself exports as a silent dummy track named "Conductor", preserving the 8-track layout.
