...
## [Unreleased]
### Features
- **Co-run surface polish.** In both co-run modes (Schwung chain-edit and Move-native): out-of-key (non-root, in-scale) melodic pads now show dim track color instead of bright; the step-button grid is blanked except Step 3, which blinks dark-grey/off as the exit affordance with its icon lit solid white; pressing Step 3 exits co-run (mirrors the Menu/Back exit). Blink runs off wall-clock so the rate matches across both modes. The Menu / Note-Session button (the secondary exit) is held solid bright white in co-run.
