...
## [Unreleased]
### Features
- **Mute is handed to Move during co-run.** While co-running Move's native instruments and drum pads (Shift+Step 3 / Track Config), pressing **Mute** now mutes the *Move* instrument or drum pad you're working with, instead of being captured by dAVEBOx. Outside co-run, and while editing a Schwung chain, Mute keeps its dAVEBOx track mute/solo behavior (in the FX picker it cedes to Move, like normal co-run). (Requires Schwung with the Mute co-run group; older Schwung simply leaves Mute with dAVEBOx as before.)
