...
### Features
- **Live Merge now has a count-in and starts from a stopped transport.** Shift + Record starts a Live Merge only when the transport is stopped (it's ignored while playing). It gives you a **1-bar count-in** (metronome + flashing step buttons), then starts playback and captures a clean take from the top of the pattern — the single-track (Track View) and all-8-tracks (Session View) modes both work this way. Press **Record** at any time to stop (its dialog now says "Rec to stop"), or during the count-in to abort.
