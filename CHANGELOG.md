...
- **Manual: Clock Follow and Clock Out documented.** Clock Follow (§17.7) explains the Move-native integration in detail — the "dAVEBOx is the sequencer, Move provides clock/tempo/voices" model, practical setup steps, and the clear-Move-clips requirement. Clock Out (§17.8) promoted to its own section. *(Both held from public release pending upstream async-external-MIDI-send PR.)*
