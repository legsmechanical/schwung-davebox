...
- **On-device and second-screen edits now show up in the browser promptly.** A large set of edits — CC automation, step trig conditions, loop points, conductor responders, clip/lane clears, drum lane mute/solo/length/direction/resolution/stretch/shift/nudge, undo/redo, clip and row copy/cut — weren't flagging the browser that anything changed, so it could show stale data for up to ~30 s (or until an unrelated edit refreshed it). They now update the browser right away.
- **Very dense clips no longer blank the browser editor.** A drum clip packed with enough hits could overflow the editor's data snapshot and produce data the browser silently rejected — leaving the editor stuck/blank for that clip until it was thinned on the device. The snapshot now always stays valid (showing as many notes as fit), so the editor keeps working.
