...
function updateTrackLEDs() {
     ...
     * using per-track S.tarpStepVel (no per-clip dimension). */
    ...
    /* Knob LEDs (CC 71-78):
     *   Session View              → White on S.activeTrack (active-track indicator)
     *   Track View + PARAM_LED_BANKS → White when param ≠ default for S.activeTrack/S.activeBank
 ...
 * "high" half of the period. S.masterPos is polled from DSP; granularity is
 * the poll interval (≈92ms). Works whether or not transport is running
 * because S.masterPos uses the free-running arp_master_tick. */
