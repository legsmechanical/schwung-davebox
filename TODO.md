...
- **[NEXT UP — prioritized] Clip/lane copy/paste loses non-zero loop start.** Copying a clip (or drum lane) that has its loop window starting at a step > 0 — paste produces a clip/lane with `loop_start = 0`, so the copied step data still lives at the original step positions but is now outside the loop window (OOB / unreachable). Fix: copy `loop_start` alongside the rest of the clip/lane data.
- **[NEXT UP — prioritized] Melodic clip with loop_start > 0 initially shows OOB region on select.** Selecting such a clip briefly displays the OOB step region; jogging left/right snaps the view back to the correct page. Drum lanes don't have this symptom. Likely an initial-page calculation that ignores `loop_start`.
